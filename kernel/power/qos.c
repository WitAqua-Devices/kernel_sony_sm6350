		...
		struct cpumask *cpus)
{
	 ...
	 * target_per_cpu is only meaningful for constraints that live in
	 * pm_qos_array; device requests are not in it, so bail out for
	 * anything else. This is the same protection the upstream
	 * dev_req flag provides.
	 */
