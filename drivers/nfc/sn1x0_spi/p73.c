...
/*
 * bus_find_device_by_fwnode() only exists from 4.20 onwards; this tree is
 * 4.19.  Open code the same lookup instead of backporting it into the driver
 * core.
 */
static int p61_match_fwnode(struct device *dev, void *fwnode)
{
	return dev_fwnode(dev) == fwnode;
}

static struct device *p61_find_device_by_fwnode(struct bus_type *bus,
						struct fwnode_handle *fwnode)
{
	return bus_find_device(bus, NULL, fwnode, p61_match_fwnode);
}
...
static int p61_dev_open(struct inode *inode, struct file *filp)
{
	...
	*p61_dev = container_of(filp->private_data,
	...
	if (p61_dev != NULL && p61_dev->nfcc_data == NULL) {
		...
		nfc_dev = p61_find_device_by_fwnode(&i2c_bus_type, &nfcc_node->fwnode);
		if (!nfc_dev) {
