...
/*
 * Must match the struct the Egis user space library passes to every ioctl.
 * FP_POWER_ONOFF takes the requested state in power_on, not in int_mode --
 * the reference driver (Motorola's rbs_fps_mmi/etxxx_fp.c, same user space)
...
static long etspi_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_POWER_ONOFF:  //0x05
			...
			power = data.power_on ? true : false;
...
//fanjiafeng5  add end
...
#endif	
	/*
	 * The Egis user space library asks the driver to take and drop the
	 * sensor's GPIOs around every session.  This driver requests them once
	 * in etspi_probe() and never releases them (etspi_release() only frees
	 * the transfer buffer), so both are a no-op here -- freeing them would
	 * make the next FP_GET_RESOURCE fail with -EBUSY.
	 *
	 * Without these two the very first ioctl of rbs_initialize() returns
	 * -ENOTTY, libRbsFlow gives up with "cpt_initialize fp_get_resource
	 * failed" and the fingerprint HAL never opens the sensor.
	 */
	/*
	 * Drive the reset line to the level user space asks for, instead of the
	 * fixed low-high pulse FP_SENSOR_RESET does.  data.int_mode carries the
	 * level (1 = high, 0 = low); the delays match etspi_reset().
	 */
	case FP_RESET_SET:
			if (copy_from_user(&data, (int __user *)arg, sizeof(data))) {
				retval = -EFAULT;
			...
			DEBUG_PRINT("EGISTEC fp_ioctl <<< FP_RESET_SET %d\n", data.int_mode);
			if (data.int_mode) {
				gpio_set_value(etspi->rstPin, 1);
				msleep(20);
			} else {
	...
	case FP_GET_RESOURCE:
			DEBUG_PRINT("EGISTEC fp_ioctl <<< FP_GET_RESOURCE\n");
		goto done;

	case FP_FREE_RESOURCE:
			DEBUG_PRINT("EGISTEC fp_ioctl <<< FP_FREE_RESOURCE\n");
		goto done;

	default:
		pr_err("%s: unhandled opcode 0x%x\n", __func__, cmd);
		retval = -ENOTTY;
		break;
	}
done:
...
/*
 * The device trees Sony ships for lena (pdx213) spell the GPIOs
 * egistec,gpio_reset / egistec,gpio_vcc_en / egistec,gpio_vdd_en, while this
 * driver was released expecting egistec,gpio_rst / egistec,gpio_pwr_en /
 * egistec,gpio_ldo1p8_en.  Accept both so the driver works with either
 * spelling instead of failing the probe outright.
 */
static int etspi_get_named_gpio(struct device_node *np, const char *name,
	const char *legacy_name)
{
	int gpio = of_get_named_gpio(np, name, 0);

	if (gpio < 0)
		gpio = of_get_named_gpio(np, legacy_name, 0);
...
//	int ret;
	...
	gpio = etspi_get_named_gpio(np, "egistec,gpio_reset", "egistec,gpio_rst");
	if (gpio < 0) {
		errorno = gpio;
		goto dt_exit;
	} else {
...
*/
#ifdef power_gpio_supply_3v3
	gpio = etspi_get_named_gpio(np, "egistec,gpio_vcc_en", "egistec,gpio_pwr_en");
	if (gpio < 0) {
		errorno = gpio;
		goto dt_exit;
	} else {
...
#endif
#ifdef egis_1v8
	gpio = etspi_get_named_gpio(np, "egistec,gpio_vdd_en", "egistec,gpio_ldo1p8_en");
	if (gpio < 0) {
		errorno = gpio;
		goto dt_exit;
	} else {
...
/*
 * The fingerprint node in the stock dtbo (soc/fp_egistec) is
 * compatible = "fp-egistec", which nothing in this table matched, so probe
 * never ran and /dev/esfp0 was never created.  libRbsFlow then failed with
 *
 *   fp_device_open [158] stat: No such file or directory
 *   RBS: g_lib_handle->rbs_initialize failed, return = 4
 *
 * The string "fp-egistec" is present in the stock Image, so the shipping
 * kernel does have this entry and the released sources are missing it.
 * Add it here rather than patching the device tree, which is used as is.
 */
